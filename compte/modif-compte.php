...
///////////////////////////////////////
  $getUser=$conn->prepare("SELECT * FROM tbl_user t WHERE t.user_id=:uuser_id"); // On obtient user avec l'ID uid
  $getUser->execute(array(':uuser_id'=>$_GET['ID']));	// // uid ID devient l'ID passer en _GET
  $rowUser=$getUser->fetch(PDO::FETCH_ASSOC); // On met le user dans un talbeau pour l'exploiter 
?>
<!DOCTYPE html>
  ...
  <body class="u-body">
    ...
    <div style="width: 30%; height: fit-content; left: 35%; top: 20%; bottom: 1%; box-shadow: 0px 4px 40px rgba(0, 0, 0, 0.50); box-sizing: border-box; position: absolute; background-color: #DDE2E3;">
        ...
        <div class="u-form u-form-1">
          ...
          <form action="../BDD/compte/db_modif.php" method="POST" class="u-clearfix u-form-custom-backend u-form-spacing-25 u-form-vertical u-inner-form" style="padding: 15px" source="custom" redirect="true">
            <input type="hidden" name="ID" value="<?php echo $ID; ?>">
            <!-- Identifiant -->
            <div class="u-form-group u-form-name">
                <label for="identifiant" class="u-form-control-hidden u-label"></label>
                <input type="text" autocomplete="off" placeholder="Identifiant" id="identifiant" name="identifiant" class="u-border-1 u-border-grey-30 u-input u-input-rectangle" required="" value="<?php echo $rowUser["username"];?>">
            </div>
            ...
            <div class="u-form-email u-form-group">
              ...
              <input type="text" autocomplete="off" placeholder="Nouveau mot de passe" id="MDP" name="MDP" class="u-border-1 u-border-grey-30 u-input u-input-rectangle" >
            </div>
            ...
            <label for="permission" class="u-label">Permission</label>
              ...
              <select id="permission"  name="permission" class="u-border-1 u-border-grey-30 u-input u-input-rectangle" required="required">
                <?php
                  echo '<option>'. $rowUser["permission"] .'</option>';
                  $reponse = $conn->query('SELECT p.etat FROM permissions p WHERE p.etat!="'. $rowUser["permission"] .'"'); 
            ...
            <div class="u-form-group u-form-partition-factor-2 u-form-select u-form-group-3">
              ...
              <select id="site" name="site" class="u-border-1 u-border-grey-30 u-input u-input-rectangle" required="required">
                ...
                <?php
                echo '<option>'. $rowUser["site"] .'</option>';
                $reponse = $conn->query('SELECT s.nom FROM site s WHERE s.Nom!="' . $rowUser["site"] . '" '); 
